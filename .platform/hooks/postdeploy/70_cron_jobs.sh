...
echo "* * * * * webapp source /opt/elasticbeanstalk/deployment/envvars && php /var/app/current/craft schedule/run --scheduleFile=@config/schedule.php 1>> /dev/null 2>&1" | sudo tee /etc/cron.d/craftnet