...
import CartHelper from '../../helpers/cart.js'
...
const getters = {
  ...
  cartIsEmpty(state) {
    if (state.cart && state.cart.lineItems.length == 0) {
      return true
    }
  ...
  addToCart({commit, state, dispatch}, newItems) {
    return new Promise((resolve, reject) => {
      dispatch('getCart')
        .then(() => {
          const cart = state.cart
          const items = CartHelper.getCartItemsData(cart)
...
const mutations = {
  ...
  updateItemQuantity(state, {itemKey, value}) {
    state.itemsQuantity[itemKey] = value
  },
