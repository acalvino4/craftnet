<template>
    <div v-if="draftProject">
        <div v-show="!isEditing">
            <div class="flex">
                ...
                <ul class="flex-1">
                    ...
                    <li v-if="draftProject.withCraftCommerce" class="mt-3">
                        &#10004; This project includes Craft Commerce
                    </li>
                    <li v-if="draftProject.screenshots.length" class="mt-3">
                ...
                <div>
                    <btn @click="$emit('edit', index)">
                        <icon icon="pencil" :size="null" class="w-3 h-3"/>
                        Edit
                    </btn>
                </div>
            </div>
        ...
        <modal v-if="isEditing" :show="isEditing" transition="fade"
               modal-type="wide">
            <template v-slot:body>
                <form class="p-4" @submit.prevent="save()">
                    <field :vertical="true" :first="true" label-for="name"
                           label="Project Name*" :errors="localErrors.name">
                        <textbox id="name" v-model="draftProject.name"
                                 :is-invalid="!!localErrors.name"/>
                    </field>
                    <field :vertical="true" label-for="role" label="Role"
                        ...
                        <textbox id="role" v-model="draftProject.role" :max="55"
                                 :is-invalid="!!localErrors.role"/>
                    </field>
                    <field :vertical="true" label-for="url" label="URL*"
                           :errors="localErrors.url">
                        <textbox id="url" v-model="draftProject.url"
                                 :is-invalid="!!localErrors.url"/>
                    </field>

                    <field :vertical="true" label-for="linkType"
                           label="Link Type" :errors="localErrors.linkType">
                        <dropdown id="linkType" v-model="draftProject.linkType"
                                  :options="options.linkType"
                                  :is-invalid="!!localErrors.linkType"/>
                    </field>

                    <field :vertical="true">
                        <checkbox id="withCraftCommerce"
                                  label="This project includes Craft Commerce"
                                  v-model="draftProject.withCraftCommerce"
                                  :checked-value="1"/>
                    </field>

                    <field :vertical="true" label="Screenshots*"
                        ...
                        <draggable v-model="draftProject.screenshots"
                                   item-key="project">
                    ...
                    <div v-if="draftProject.screenshots.length <= 5"
                         class="mt-4">
                        <input type="file" accept=".jp2,.jpeg,.jpg,.jpx"
                               @change="screenshotFileChange"
                               ref="screenshotFiles" class="hidden" multiple="">
