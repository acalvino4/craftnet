    ...
    <div class="space-y-6">
            ...
            <template v-if="license">
                <div class="md:flex -mx-4">
                    <div class="md:w-1/2 px-4">
                        <dl>
                            <dt>Edition</dt>
                            <dd>{{ $filters.capitalize(license.edition) }}</dd>

                            <dt>License Key</dt>
                            <dd>
                                <code>{{ license.key.slice(0, 10) }}…</code>
                                <a href="#copy" class="ml-2"
                                   @click="copyLicense">Copy</a>
                                <a :href="downloadLicenseUrl" class="ml-2"
                            ...
                            <dd>
                                ...
                                <form v-if="domainEditing"
                                      @submit.prevent="saveDomain()">
                                    <textbox id="domain"
                                             v-model="licenseDraft.domain"
                                             @input="domainChange"/>
                                    <btn kind="primary" type="submit"
                                         :class="{disabled: !domainValidates}"
                                         :disabled="!domainValidates">Save
                                    </btn>
                                    <btn @click="cancelEditDomain()">Cancel
                                    </btn>
                    ...
                    <div class="md:w-1/2 px-4">
                        <dl>
                            ...
                            <dd>
                                <template
                                    v-if="license.expirable && license.expiresOn">
                                    {{
        ...
        <pane v-if="license.expirable && license.expiresOn">
            ...
            <div class="text-grey-dark">
                <p>Renewal Price:
                    <strong>{{ $filters.currency(license.editionDetails.renewalPrice) }}/year</strong>.
                </p>
        ...
        <pane>
            ...
            <template v-if="license.expirable && license.expiresOn">
                <btn @click="showRenewCmsLicenseModal = true">Renew your license</btn>

                <renew-cms-license-modal
                    :show-modal="showRenewCmsLicenseModal"
                    :license="license"
                    @close="showRenewCmsLicenseModal = false"
                />
            </template>
        </pane>
    </div>
...
import RenewCmsLicenseModal from "./renew-licenses/RenewCmsLicenseModal";

export default {
    props: ['license'],

    components: {
        RenewCmsLicenseModal,
    ...
    data() {
        return {
            ...
            renewLicenses: false,
            showRenewCmsLicenseModal: false,
        }
    },
    ...
    computed: {
        ...
        formattedLicense() {
            ...
            let formattedValue = this.$filters.formatCmsLicense(value)
            return formattedValue
        },

        downloadLicenseUrl() {
            return Craft.actionUrl + '/craftnet/console/cms-licenses/download&id=' + this.license.id
        },
    },
