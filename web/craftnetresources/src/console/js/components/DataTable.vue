    ...
    <template v-if="filterable">
      <div class="mb-4">
        <input
          v-model="searchQuery"
          type="text"
          class="rounded text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 w-full"
    ...
    <div class="overflow-x-scroll">
      <table class="w-full">
        ...
        <template v-if="responseData && responseData.data">
          <tr
            v-for="(dataItem, dataItemKey) in responseData.data"
            :key="'tr-' + dataItemKey">
            <td
              v-for="(column, columnKey) in columns"
              :key="'response-data-table-' + columnKey">
              <slot
                :name="column"
                :row="dataItem">
                <template v-if="dataItem[column]">{{
                    dataItem[column]
                  }}
                </template>
              </slot>
          ...
          <template v-if="url">
            <div class="flex space-x-3 relative">
              <btn
                ...
                @click="goToPreviousPage">← Previous Page
              </btn>
              <btn
                :disabled="currentPage >= responseData.last_page"
                @click="goToNextPage">Next Page →
              </btn>
            </div>
          </template>
        </div>
