<template>
    <modal-headless :isOpen="show" @close="$emit('close')">
        ...
        <template v-slot:footer>
            <div class="flex justify-end items-center space-x-2">
                ...
                <btn ref="cancelBtn" :disabled="loading"
                     @click="$emit('close')">Cancel
                </btn>
                <template v-if="subscriptionMode === 'subscribe'">
                    <btn ref="submitBtn" kind="primary"
                         :disabled="!card || loading" @click="subscribePlan()">
                        Subscribe to this plan
                    </btn>
                </template>
                <template v-else>
                    <btn ref="submitBtn" kind="primary"
                         :disabled="!card || loading" @click="switchPlan()">
