      ...
      <div class="flex-1">
        ...
        <template v-else>
          <template v-if="!showForm && billingAddress">
            <ul
              v-if="billingAddress.firstName || billingAddress.lastName || billingAddress.address1 || billingAddress.address2 || billingAddress.city || billingAddress.country || billingAddress.businessName || billingAddress.state || billingAddress.zipCode"
              class="list-reset">
            ...
            <p
              v-else
              class="text-secondary">Billing address not defined.</p>
          </template>
        </template>
      </div>
    ...
    <template v-if="!loading">
      <form
        ...
        <textbox
          id="city"
          label="City"
          v-model="invoiceDetailsDraft.city"
          :errors="errors.city" />
        <dropdown
          id="country"
          label="Country"
          v-model="invoiceDetailsDraft.country"
          :options="countryOptions"
          @input="onCountryChange" />
        <dropdown
          id="state"
          label="State"
          v-model="invoiceDetailsDraft.state"
          :options="stateOptions(invoiceDetailsDraft.country)" />
        <textbox
          id="zipCode"
          label="Zip Code"
          v-model="invoiceDetailsDraft.zipCode"
