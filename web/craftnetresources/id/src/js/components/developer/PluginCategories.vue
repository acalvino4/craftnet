<template>
  <div>
    <field
      label="Categories"
      :instructions="'Pick up to '+maxCategories+' categories. ('+ pluginDraft.categoryIds.length +'/'+maxCategories+' selected)'">
      ...
      <template v-else>
        <draggable v-model="pluginDraft.categoryIds">
          <div
            class="alert float-left clearfix mb-3 mr-2 px-3 py-2"
        ...
        <div>
          <div
            ...
            <btn
              class="mb-2 mr-2"
              icon="plus"
              :disabled="pluginDraft.categoryIds.length >= maxCategories"
              outline
