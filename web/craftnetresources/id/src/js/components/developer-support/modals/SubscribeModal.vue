  ...
  <modal
    ...
    <template
      ...
      <template v-else>
        ...
        <p>Your plan will switch to the {{ selectedPlan.name }} tier at the end
          of the billing cycle.</p>
      </template>

      <template v-if="!card">
        <p>You must add a credit card to your account at
          <a @click="goToBilling">Account → Billing</a> before signing up for a
          support plan.</p>
      </template>
        ...
        <tr>
          ...
          <td class="text-right">
            ...
            <small class="text-grey-dark">Then
              {{ subscriptionInfoPlan.cost.recurring|currency }} every
              month</small>
          </td>
        </tr>
        </tbody>
