      ...
      <div
        ...
        <div class="flex">
          <ul class="flex-1 list-reset">
            <li
              v-if="project.name"
              class="mb-3"><strong class="text-2xl">{{
                project.name
              }}</strong></li>
            <li v-if="project.role">Role: {{ project.role }}</li>
            <li v-if="project.url">{{ linkTypeDisplay }} Link: <a
              :href="project.url"
              target="_blank">{{ project.url }}</a></li>
            <li
              v-if="project.withCraftCommerce"
              class="mt-3">&#10004; This project includes Craft Commerce
            </li>
            <li
              v-if="project.screenshots.length"
              class="mt-3"><strong>Screenshots</strong></li>
            <li class="flex">
              <div
                v-for="(screenshot, index) in project.screenshots"
                :key="index"
                class="p-1 mt-2 inline-block bg-grey-lightest flex align-middle justify-center"
    ...
    <modal
      ...
      <div
        ...
        <dropdown
          id="linkType"
          label="Link Type"
          v-model="project.linkType"
          :options="options.linkType"
          :errors="localErrors.linkType" />
        <checkbox
          id="withCraftCommerce"
          label="This project includes Craft Commerce"
          v-model="project.withCraftCommerce"
          :checked-value="1" />

        <label>Screenshots<span class="text-red">*</span></label>
        <p class="instructions">1 to 5 JPG screenshots required with a 12:7
          aspect ratio. 1200px wide will do. Drag to re-order.</p>
        ...
        <div
          ...
          v-for="(error, errorKey) in localErrors.screenshots"
          :key="errorKey">{{ error }}
        </div>
        ...
        <div
          v-if="project.screenshots.length <= 5"
          class="mt-4">
          <input
            type="file"
