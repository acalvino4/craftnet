  ...
  <div>
    <div class="flex justify-between mb-2">
      ...
      <div>
        <btn
          kind="primary"
          icon="plus"
          to="/developer/add-plugin">Add a plugin
        </btn>
      </div>
    </div>
    ...
    <template v-else>
      <div
        ...
        class="card card-table responsive-content">
        <table class="table">
          <thead>
          <tr>
            <th></th>
          ...
          <tr
            ...
            <td>
              ...
              <template v-else>
                ...
                <span
                  v-if="plugin.pendingApproval"
                  class="text-secondary">In Review</span>
                <template v-else>
                  <span
                    v-if="plugin.lastHistoryNote && plugin.lastHistoryNote.devComments"
                    class="text-warning">Changes requested</span>
                  <span
                    v-else
                    class="text-secondary">Prepare for submission</span>
                </template>
              </template>
            </td>
          </tr>
