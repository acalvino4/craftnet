      ...
      <template v-if="license">
        ...
        <div class="card mb-3">
          <div class="card-body">
            ...
            <template v-if="license.pluginLicenses.length > 0">
              <p class="text-secondary mb-4">Plugin licenses attached to this
                Craft CMS license.</p>
              ...
              <template v-if="hasPluginRenewals && !(license.expirable && license.expiresOn)">
                <btn @click="showRenewLicensesModal('cms')">Renew your plugin
                  licenses…
                </btn>
              </template>
            </template>
            <template v-else>
              <p class="text-secondary mb-4">No plugin licenses are attached to
                this Craft CMS license.</p>
            </template>
          </div>
        </div>
        ...
        <div class="card card-danger mb-3">
          ...
          <div class="card-body">
            ...
            <p>Release this license if you no longer wish to use it, so that it
              can be claimed by someone else.</p>
            <div>
              <btn
                kind="danger"
                @click="releaseCmsLicense()">Release License
              </btn>
            </div>
          </div>
        </div>
